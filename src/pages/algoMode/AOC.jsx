...
//     // ==========================================
//     // ⚙️ THE STRATEGY MANAGER (POSTGRES DATABASE)
//     // ==========================================
//     const [savedProfiles, setSavedProfiles] = useState([]);
//     const [activeProfileId, setActiveProfileId] = useState('');
//     const [strategyConfig, setStrategyConfig] = useState(null);

//     // 1. पेज लोड होते ही डेटाबेस से स्ट्रेटेजी लाएं
//     useEffect(() => {
//         const fetchStrategies = async () => {
//             try {
//                 // पोर्ट 5500 का इस्तेमाल (तुम्हारे Node.js बैकएंड के हिसाब से)
//                 const res = await axios.get('http://localhost:5500/api/strategies/list');
...
//     // 2. जब यूज़र ड्रॉपडाउन से कोई स्ट्रेटेजी चुने
//     const handleProfileChange = (e) => {
//         const id = e.target.value;
//         setActiveProfileId(id);
//         const profile = savedProfiles.find(p => p.id === Number(id));
//         if (profile) {
//             setStrategyConfig(profile.config);
//             console.log(`✅ Loaded Profile: ${profile.profile_name}`, profile.config);
//         }
//     };
...
//     // 1. ONE-TIME INITIAL LOAD (टेबल का ढांचा बनाने के लिए)
//     const fetchInitialAOC = async () => {
//         try {
//             const res = await axios.get(`${API_BASE_URL}/api/live/aoc`, { 
//                 params: { symbol: 'NIFTY', expiry: expiry, _t: Date.now() } 
//             });
            
//             if (res.data.success && res.data.chain.length > 0) {
//                 // Table ka skeleton set karo
//                 setData({ spotPrice: res.data.spotPrice, chain: res.data.chain });
                
//                 // 🎯 THE FIX: Skeleton banne ke turant baad backend ko subscribe ka signal bhejo
//                 console.log("Sending subscription request for expiry:", expiry);
//                 socket.emit('subscribe-options-chain', { 
//                     expiry: expiry,
//                     fallbackSpot: res.data.spotPrice // 👈 यह नया जोड़ा है
//                 });
//             }
//         } catch (error) {
//             console.error("Live AOC Fetch Error:", error);
...
//     // Sirf ek baar API call hogi, loop mein nahi!
//     fetchInitialAOC();

//     // 2. 🚀 SOCKET LISTENER (Tick-by-Tick Update)
...
//             }));
//         } else {
//             // 🎯 Option Chain (CE/PE) LTP Update
//             setData(prevData => {
//                 if (!prevData.chain || prevData.chain.length === 0) return prevData;

//                 // Naya array banayenge kyunki React mein direct state mutate nahi karte
//                 const updatedChain = prevData.chain.map(row => {
//                     let updatedRow = { ...row };
                    
//                     // Agar tick ka Token (securityId) CE ke token se match kare
//                     if (row.CE && row.CE.token === tick.securityId) {
//                         updatedRow.CE = { ...row.CE, last_price: tick.ltp };
//                     }
                    
//                     // Agar tick ka Token PE ke token se match kare
//                     if (row.PE && row.PE.token === tick.securityId) {
//                         updatedRow.PE = { ...row.PE, last_price: tick.ltp };
//                     }
                    
//                     return updatedRow;
//                 });

//                 return { ...prevData, chain: updatedChain };
//             });
//         }
//     });
...
//                             className="bg-white border border-gray-300 text-gray-700 text-xs font-extrabold rounded-full px-3 py-1.5 focus:outline-none focus:border-blue-500 shadow-sm cursor-pointer w-40 truncate hover:bg-gray-50 transition-colors"
//                         >
//                             <option value="" disabled hidden>⚙️ Select Strategy</option>
...
//                             marketMetrics={marketMetrics} chainData={data.chain} showMagicalLines={showMagicalLines}  showOrderLines={showOrderLines} sniperRules={sniperRules} sniperMode={sniperMode} maxShiftPts={maxShiftPts}  spotPrice={data.spotPrice} showReport={showReport} setShowReport={setShowReport} marketActionText={marketSentiment.text} strategyConfig={strategyConfig}/>
//                     </div>
//                 </div>
...
import BlinkingCell from '../../components/algoComponents/Aoc/BlinkingCell';
...
useEffect(() => {
    ...
    //             // tick.securityId को String में बदल लो ताकि मैच करने में गड़बड़ ना हो
    //             const tickToken = String(tick.securityId);

    //             // टेबल को अपडेट करो
    //             const updatedChain = prevData.chain.map(row => {
    //                 let updatedRow = { ...row };
                    
    //                 // CE मैच (String comparison)
    //                 if (row.CE && String(row.CE.token) === tickToken) {
    //                     updatedRow.CE = { ...row.CE, ltp: tick.ltp }; // 👈 ध्यान दो: last_price की जगह ltp कर दिया
    //                 }
                    
    //                 // PE मैच (String comparison)
    //                 if (row.PE && String(row.PE.token) === tickToken) {
    //                     updatedRow.PE = { ...row.PE, ltp: tick.ltp }; // 👈 यहाँ भी ltp कर दिया
    //                 }
    ...
    socket.on('market-tick', (tick) => {
        ...
        // 🎯 Nifty Spot Update
        if (tick.securityId === "13" && tick.ltp) {
            setData(prevData => ({
                ...prevData,
                spotPrice: tick.ltp
            }));
        } 
        // 🎯 Option Chain (CE/PE) Update
        else {
            setData(prevData => {
                if (!prevData.chain || prevData.chain.length === 0) return prevData;
                ...
                const updatedChain = prevData.chain.map(row => {
                    ...
                    // CE Update
                    if (row.CE && String(row.CE.token) === tickToken) {
                        updatedRow.CE = { ...row.CE };
                        if (tick.ltp !== undefined) updatedRow.CE.ltp = tick.ltp;
                        if (tick.volume !== undefined) updatedRow.CE.volume = tick.volume;
                        if (tick.oi !== undefined) updatedRow.CE.oi = tick.oi;
                    }
                    
                    // PE Update
                    if (row.PE && String(row.PE.token) === tickToken) {
                        updatedRow.PE = { ...row.PE };
                        if (tick.ltp !== undefined) updatedRow.PE.ltp = tick.ltp;
                        if (tick.volume !== undefined) updatedRow.PE.volume = tick.volume;
                        if (tick.oi !== undefined) updatedRow.PE.oi = tick.oi;
                    }
                                    ...
                                    return data.chain.reduce((acc, row, idx) => {
                                        ...
                                        const rowElement = (
                                            <tr key={`strike-${idx}`} ref={isATM ? atmRowRef : null} className={`border-b hover:bg-gray-50`}>
                                                ...
                                                {/* 🚀 THE FIX: CE का ब्लिंकिंग LTP */}
                                                <td className={`border p-0 h-[38px] font-bold text-gray-800 truncate ${callStyle}`}>
                                                    <BlinkingCell value={row.CE?.ltp} />
                                                </td>

                                                <td className={`border border-gray-300 p-0 font-extrabold text-[12px] h-[38px] truncate ${isATM ? 'bg-gray-200 text-gray-800 shadow-inner' : 'bg-gray-200 text-gray-800'}`}>{row.strike}</td>

                                                {/* 🚀 THE FIX: PE का ब्लिंकिंग LTP */}
                                                <td className={`border p-0 h-[38px] font-bold text-gray-800 truncate ${putStyle}`}>
                                                    <BlinkingCell value={row.PE?.ltp} />
                                                </td>
...
//                 if (res.data.success && res.data.chain.length > 0) {
//                     // सिर्फ डेटा सेट करो, CustomChart खुद स्पॉट प्राइस को कैच करके कैंडल अपडेट कर लेगा!
//                     setData({ spotPrice: res.data.spotPrice, chain: res.data.chain });
//                 }
//             } catch (error) {
//                 console.error("Live AOC Fetch Error:", error);
...
//         // 10 सेकंड में नया डेटा (Dhan API को ब्लॉक होने से बचाने के लिए)
//         const intervalId = setInterval(fetchLiveAOC, 10000); 
