...
import { UploadCloud, FileText, Loader, Database, AlertCircle } from 'lucide-react';
...
const ImportNotion = () => {
  ...
  const parseMarkdownData = (text) => {
    ...
    lines.forEach(line => {
      ...
      if (parts.length >= 2) {
        ...
        // Clean Numbers
        if (['NETP&L', 'GROSSP&L', 'BUYPRICE', 'SELLPRICE', 'QUANTITY', 'TOTALBROKERAGE', 'P&LINPOINT'].includes(key)) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const allMdFiles = allPaths.filter(path => path.endsWith('.md') && !path.startsWith('__MACOSX') && !path.includes('Export Summary'));
      ...
      if(validTrades.length > 0) {
        addLog(`🎉 Ready to import ${validTrades.length} trades. Click Upload button.`, 'success');
      } else {
  ...
  const handleFinalUpload = async () => {
      ...
      const getStandardPair = (rawName) => {
          ...
          const mappings = {
              'MIDCAP': 'MIDCPNIFTY', 'NIFTY': 'NIFTY', 'BANKNIFTY': 'BANKNIFTY',
      ...
      for (const trade of parsedData) {
          ...
          const mongoObject = {
              ...
              mode: trade.raw_data['TRADETYPE']?.toUpperCase() === 'PAPER' ? 'BACKTEST' : 'LIVE', 
              ...
              result: (safeNum(trade.raw_data['NETP&L']) > 0) ? 'TARGET HIT' : 'STOPLOSS HIT',
      ...
      try {
          ...
          await axios.post('http://localhost:5000/api/trades/bulk-import', finalPayload);
          addLog("✅ ALL DATA IMPORTED SUCCESSFULLY!", 'success');
          alert("Import Complete!");
          window.location.reload(); 
      } catch (err) {
          console.error(err);
          addLog(`❌ Backend Error: ${err.response?.data?.message || err.message}`, 'error');
      }
    ...
    <div className="p-6 bg-white rounded-xl shadow border border-slate-100">
      ...
      {!uploading ? (
        <div className="mb-6 p-8 border-2 border-dashed border-blue-300 rounded-lg bg-blue-50 flex flex-col items-center justify-center text-center">
            <input type="file" accept=".zip" onChange={handleFileUpload} className="hidden" id="zip-upload" disabled={loading}/>
            <label htmlFor="zip-upload" className="cursor-pointer flex flex-col items-center w-full">
            {loading ? <Loader className="animate-spin text-blue-600 mb-2" size={32}/> : <FileText className="text-blue-500 mb-2" size={32}/>}
            <span className="text-sm font-bold text-slate-700">{loading ? "Analyzing ZIP..." : "Select Notion Export ZIP"}</span>
            </label>
        </div>
      ) : (
          <div className="mb-6 p-8 bg-slate-50 rounded-lg text-center">
              ...
              <h3 className="text-lg font-bold text-slate-700">Uploading Images & Data...</h3>
      ...
      {(stats.trades > 0) && (
        <div className="flex gap-4 mb-4">
           ...
           <div className="flex-1 bg-blue-50 p-3 rounded border border-blue-100">
             ...
             <h3 className="text-xl font-bold text-slate-800">{stats.imagesFound}</h3>
           </div>
        </div>
      )}

      <div className="bg-slate-900 text-slate-300 p-3 rounded-lg h-32 overflow-y-auto text-xs font-mono mb-4">
        {logs.map((l, i) => <p key={i} className={`mb-1 ${l.type === 'error' ? 'text-red-400' : l.type === 'success' ? 'text-green-400' : 'text-slate-300'}`}>{'> ' + l.msg}</p>)}
      </div>
