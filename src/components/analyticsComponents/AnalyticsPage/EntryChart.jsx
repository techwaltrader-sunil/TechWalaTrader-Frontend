...
const EntryChart = ({ data, metricMode, resultFilter }) => {
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition">
      <h3 className="text-slate-700 font-bold mb-4 flex items-center gap-2">
        {metricMode === "COUNT" ? <Hash size={18} className="text-purple-500" /> : <DollarSign size={18} className="text-purple-500" />}
        {metricMode === "COUNT" ? "Most Used Entry Types" : resultFilter === "Brokerage" ? "Entry Type Charges" : "Entry Type Performance"}
      </h3>
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart layout="vertical" data={data} margin={{ top: 5, right: 30, left: 0, bottom: 5 }}>
            <XAxis type="number" hide />
            <YAxis dataKey="name" type="category" width={130} tickMargin={5} interval={0} tick={{ fill: "#64748b", fontSize: 9, fontWeight: "700", textAnchor: "end" }} />
