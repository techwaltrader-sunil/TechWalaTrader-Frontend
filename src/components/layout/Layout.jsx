...
import { useLocation } from "react-router-dom"; // 🔥 Naya Import
import { Menu, X, Bell, ShieldAlert } from "lucide-react";
...
const Layout = ({ children }) => {
  ...
  // 🔥 Current URL track karne ke liye
  const location = useLocation();

  // 🔥 URL ke hisab se Sidebar ka mode Auto-Switch karne ka logic
  useEffect(() => {
    const algoPaths = [
    ...
    const isCurrentPathAlgo = algoPaths.some(path => location.pathname.startsWith(path));
    
    // State update kar dega
    setIsAlgoMode(isCurrentPathAlgo);
  }, [location.pathname]); // Jab bhi URL change hoga, ye chalega
