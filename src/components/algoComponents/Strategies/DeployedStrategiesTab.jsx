...
//     // 🔥 1. NAYA STATE: Live Spot Prices ke liye
//     const [liveSpots, setLiveSpots] = useState({}); 

//     // API se data mangwana (With Polling)
//     useEffect(() => {
//         // ... (Aapka purana polling wala code same rahega)
//     }, []);
...
//         // 🔥 2. NAYA SOCKET LISTENER: Jo live spot price ko sunega
//         // (Aap apne backend ke event name ke hisab se isko adjust kar lena)
//         socket.on('live-spot-update', (spotData) => {
//             // Maan lijiye backend se { symbol: 'NIFTY', ltp: 24350.65 } aa raha hai
//             if (spotData && spotData.symbol && spotData.ltp) {
//                 setLiveSpots(prev => ({
//                     ...prev,
//                     [spotData.symbol]: spotData.ltp
//                 }));
//             }
//         });
...
//                                         {(() => {
//                                             // 1. Strategy ka base symbol nikalna (jaise "NIFTY")
//                                             let baseSymbol = "NIFTY 50";
//                                             if (dep.executedLegs && dep.executedLegs.length > 0) {
//                                                 baseSymbol = String(dep.executedLegs[0].symbol).split(' ')[0];
//                                             }
                                            
//                                             // 2. Live spot check karo (jo socket se aayega), agar nahi hai toh fallback mein entrySpotPrice dikhao
//                                             const currentLiveSpot = liveSpots[baseSymbol] || dep.sessionState?.entrySpotPrice;

//                                             return (
//                                                 <StrategyPayoffChart 
//                                                     legs={dep.executedLegs} 
//                                                     liveSpot={currentLiveSpot} // 🔥 Yahan ab live value jayegi
//                                                     livePnl={currentPnl}
...
//     // 🔥 1. STATE: Live Spot Prices ke liye
//     const [liveSpots, setLiveSpots] = useState({}); 
...
//     // 🔥 2. SINGLE SOCKET CONNECTION (Duplicate hata diya gaya hai)
//     useEffect(() => {
//         const socket = io(`${import.meta.env.VITE_API_URL}`);
...
//         socket.on('live-spot-update', (spotData) => {
//             // 🔥 DEBUG CONSOLE: Browser me F12 daba kar check karein
//             // console.log("Live Spot Received from Backend:", spotData);
            
//             // Backend se ab direct object aa raha hai: { "NIFTY 50": 24350, "BANKNIFTY": 51000 }
//             if (spotData && typeof spotData === 'object') {
//                 setLiveSpots(prev => ({
//                     ...prev,
//                     ...spotData // Naya data purane state me merge ho jayega
//                 }));
//             }
//         });
...
//                                         {/* 🔥 NEW PAYOFF CHART TOGGLE SECTION 🔥 */}
//                                         {(() => {
//                                             // Strategy ka base symbol nikalna (e.g. "NIFTY")
//                                             let baseSymbol = "NIFTY";
//                                             if (dep.executedLegs && dep.executedLegs.length > 0) {
//                                                 baseSymbol = String(dep.executedLegs[0].symbol).split(' ')[0];
//                                             }
                                            
//                                             // 🔥 3. Robust Symbol Matching: "NIFTY" ya "NIFTY 50" dono check karega
