...
const StrategyCard = ({ 
  ...
  const getStrategyLabel = () => {
      ...
      // 2. Equity/Futures Segment Handling
      if (isEquity) {
          const action = (strategy.equityAction || 'BUY').toUpperCase();
          return action === 'BUY' 
  ...
  const dirLabel = getStrategyLabel();
  ...
  return (
    ...
    `}>
      ...
      <div className="flex justify-between items-start mb-4">
          ...
          {/* 🔥 THE FIX: LABEL & MENU BUTTON ALIGNED HERE 🔥 */}
          <div className="relative flex items-center gap-2">
              
              {/* DIRECTION LABEL */}
              {dirLabel && (
                  <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded uppercase tracking-widest border border-transparent shadow-sm ${dirLabel.color}`}>
                      {dirLabel.text}
                  </span>
              )}
