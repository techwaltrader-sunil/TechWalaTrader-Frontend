...
// // ==========================================
// const CustomChart = ({ symbol = 'NIFTY', date, timeframe, dataRange, time, playbackSpeed, aocStats, marketMetrics, chainData, showMagicalLines, showOrderLines, sniperRules, sniperMode, maxShiftPts, spotPrice, showReport, setShowReport, marketActionText, strategyConfig }) => {
...
//         setOpenPositions,   // 👈
//         processPnL          // 👈 
//     } = useGhostEngine(chartRef, symbol, setToastData, sniperMode, sniperRules, maxShiftPts, setAlertConfigModal, setDeleteAlertModal, chartContainerRef, chainData, marketActionText, strategyConfig);
...
//     // 2. 🎨 THE VISUAL SYNC: KLineCharts पर डायनामिक EMA लाइन ड्रा करना
//     useEffect(() => {
//         if (!chartRef.current || !strategyConfig) return;

//         try {
//             // 1. पहले पुरानी लाइन को हटाओ
//             chartRef.current.removeIndicator('candle_pane', 'EMA');
            
//             // 2. तुम्हारे भेजे हुए ऑफिशियल डॉक्स के हिसाब से एकदम सही सिंटैक्स
//             chartRef.current.createIndicator({
//                 name: 'EMA',
//                 paneId: 'candle_pane', // 👈 THE FIX: तुमने सही पकड़ा, इसे यहीं अंदर होना चाहिए!
//                 calcParams: [strategyConfig.emaPeriod],
//                 styles: {
//                     lines: [{ color: '#2962FF', size: 2, style: 'solid' }]
//                 }
//             });
            
//             console.log(`📈 Chart Updated: EMA ${strategyConfig.emaPeriod} Drawn on Main Pane`);
//         } catch (error) {
//             console.error("Error drawing indicator:", error);
//         }
//     }, [strategyConfig]);
...
//     // ==========================================
//     // 🔄 THE MANUAL SYNC ENGINE (रिफ्रेश बटन का लॉजिक)
//     // ==========================================
//     const handleSyncChart = () => {
//         if (!chartRef.current || base1mData.length === 0 || appMode !== 'historical') return;

//         // 1. यूज़र का मौजूदा ज़ूम और स्क्रॉल सेव करें
//         let currentOffset = typeof chartRef.current.getOffsetRightDistance === 'function' ? chartRef.current.getOffsetRightDistance() : 0;
//         let currentBarSpace = typeof chartRef.current.getBarSpace === 'function' ? chartRef.current.getBarSpace() : 0;
...
//                 // ✂️ कैंडल्स को सिमुलेटर के टाइम पर काटो
...
//                 // 2. ज़ूम और स्क्रॉल को वापस पुरानी जगह पर लगा दें
//                 setTimeout(() => {
//                     if (chartRef.current) {
//                         if (currentBarSpace > 0 && typeof chartRef.current.setBarSpace === 'function') {
//                             chartRef.current.setBarSpace(currentBarSpace);
//                         }
//                         if (typeof chartRef.current.setOffsetRightDistance === 'function') {
//                             chartRef.current.setOffsetRightDistance(currentOffset);
//                         }
//                     }
//                 }, 10); 
//             },
//             subscribeBar: ({ callback }) => { liveUpdateCallbackRef.current = callback; },
//             unsubscribeBar: () => { liveUpdateCallbackRef.current = null; }
//         });

//         // 3. चार्ट को रिफ्रेश करें!
//         if (typeof chartRef.current.clearData === 'function') {
...
//                 {/* ========================================================= */}
//                 {/* 🔄 THE MAGIC SYNC BUTTON (सिर्फ Historical Mode में दिखेगा) */}
//                 {/* ========================================================= */}
//                 {appMode === 'historical' && (
//                     <button 
//                         onClick={handleSyncChart}
//                         title={`Sync Chart to ${time}`}
//                         // 👇 THE FIX: 'right-16' हटाकर 'left-20' लगा दिया है!
//                         className="absolute bottom-8 left-0 z-[60] flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-full shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_6px_16px_rgba(0,0,0,0.4)] transition-all font-bold text-sm border border-indigo-400 active:scale-95 opacity-80 hover:opacity-100"
//                     >
//                         {/* रिफ्रेश / सिंक आइकॉन */}
...
const CustomChart = ({ symbol = 'NIFTY', date, timeframe, dataRange, time, playbackSpeed, aocStats, marketMetrics, chainData, showMagicalLines, showOrderLines, sniperRules, sniperMode, maxShiftPts, spotPrice, showReport, setShowReport, marketActionText, strategyConfig }) => {
    ...
    useEffect(() => {
        ...
        const crosshairHandler = (data) => {
            ...
            try {
                ...
                // 🎯 THE FIX: KLineChart v10 Smart Data Parsing
                let rawPrice = 0;
                
                if (typeof val === 'number') {
                    rawPrice = val; // अगर सीधा नंबर आया
                } else if (Array.isArray(val)) {
                    rawPrice = val[0]?.value ?? val[0] ?? 0; // अगर ऐरे आया
                } else if (val !== null && typeof val === 'object') {
                    rawPrice = val.value ?? 0; // अगर ऑब्जेक्ट आया
                }

                // पक्का करें कि प्राइस एक वैलिड नंबर ही हो, वरना 0 सेट करें
    ...
    useEffect(() => {
        // यह जादू सिर्फ तभी चलेगा जब ऐप 'Live' मोड में हो!
        if (appMode !== 'historical' && spotPrice > 0 && chartRef.current) {
            
            if (liveUpdateCallbackRef && liveUpdateCallbackRef.current) {
                // 1. चार्ट का सारा मौजूदा डेटा निकालें
                const dataList = chartRef.current.getDataList();
                
                // 2. अगर चार्ट में पहले से कैंडल्स मौजूद हैं
                if (dataList && dataList.length > 0) {
                    // आख़िरी कैंडल को पकड़ें
                    const lastCandle = dataList[dataList.length - 1];
                    
                    // 3. पुरानी कैंडल को ही नए स्पॉट प्राइस के साथ अपडेट करें
                    const updatedCandle = {
                        ...lastCandle, 
                        close: spotPrice,
                        // अगर नया प्राइस हाई से ज्यादा है, तो बत्ती ऊपर खींचो
                        high: Math.max(lastCandle.high, spotPrice), 
                        // अगर नया प्राइस लो से कम है, तो बत्ती नीचे खींचो
                        low: Math.min(lastCandle.low, spotPrice)
                        // timestamp वही रहेगा ताकि कैंडल अपडेट हो, नई कैंडल न बने
                    };
                    
                    // 4. KLineChart को पूरी कैंडल (OHLC) वापस दे दें
                    liveUpdateCallbackRef.current(updatedCandle);
                } else {
                    // अगर चार्ट बिलकुल खाली है, तो एक नई "बेस" कैंडल बना दें
                ...
                {crosshairBtn.visible && !tvMenu.visible && (
                    ...
                    >
                        ...
                        <div 
                            ...
                            style={{ width: '64px' }} 
                        >
                            {/* 🎯 THE FIX: Number() में रैप कर दिया गया है */}
                            {Number(crosshairBtn.price || 0).toFixed(2)}
                        </div>
                    </div>
                )}
                ...
                {tvMenu.visible && (
                    <>
                        ...
                        >
                            {/* Header (Price) */}
                            <div className="px-3 py-2 border-b border-gray-100 font-bold flex justify-between items-center text-blue-600 bg-blue-50/50">
                                <span className="text-[13px]">{Number(tvMenu.price || 0).toFixed(2)}</span>
                                <X 
                                    size={14} 
                                    className="cursor-pointer text-gray-400 hover:text-red-500" 
                                    onClick={() => {
