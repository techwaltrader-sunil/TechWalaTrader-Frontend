...
import { 
  ChevronDown, ChevronUp, FileText, ArrowLeft, ArrowRight, ExternalLink 
} from 'lucide-react';
    ...
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm dark:shadow-lg flex flex-col transition-colors duration-300">
      ...
      <div className="p-6 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 transition-colors">
        <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Daily trade breakdown and logs</p>
        </div>
      ...
      <div className="flex-1 overflow-x-auto">
        <div className="min-w-[800px]"> 
            ...
            {currentData.map((group) => (
                <div key={group.date} className="border-b border-gray-200 dark:border-slate-800 last:border-0 transition-colors">
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={`p-2 rounded-full transition-colors ${expandedDate === group.date ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400' : 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400'}`}>
                                {expandedDate === group.date ? <ChevronUp size={18}/> : <ChevronDown size={18}/>}
                            </div>
                            <div>
                                    ...
                                    {group.trades.map((trade, idx) => (
                                        <tr key={idx} className="border-b border-gray-200 dark:border-slate-800/30 last:border-0 hover:bg-gray-100 dark:hover:bg-slate-900/50 transition-colors">
                                            ...
                                            <td className="py-3 pl-2">
                                                <p className="text-sm font-bold text-gray-900 dark:text-white">{trade.symbol}</p>
                                                <p className="text-[10px] text-gray-500 dark:text-gray-500">Weekly Expiry</p>
                                            </td>
                                            ...
                                            {/* Entry */}
                                            <td className="py-3">
                                                <p className="text-sm font-bold text-gray-800 dark:text-gray-200">₹{trade.entryPrice}</p>
                                                <p className="text-[10px] text-gray-500 dark:text-gray-500">{trade.details?.entryTime || "09:15:00"}</p>
                                            </td>

                                            {/* Exit */}
                                            <td className="py-3">
                                                <p className="text-sm font-bold text-gray-800 dark:text-gray-200">₹{trade.exitPrice}</p>
                                                <p className="text-[10px] text-gray-500 dark:text-gray-500">{trade.details?.exitTime || "15:30:00"}</p>
                                            </td>

                                            {/* P&L */}
                                            <td className={`py-3 text-right font-bold text-sm ${trade.pnl >= 0 ? 'text-emerald-600 dark:text-emerald-500' : 'text-rose-600 dark:text-rose-500'}`}>
                                                {trade.pnl >= 0 ? '+' : '-'}{fmt(trade.pnl)}
                                            </td>

                                            {/* Exit Type */}
                                            <td className="py-3 text-right pr-2">
                                                <a href="#" className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 flex items-center justify-end gap-1 transition-colors">
            ...
            {groupedData.length === 0 && (
                <div className="p-8 text-center text-gray-500 dark:text-gray-500">No transactions found.</div>
            )}
        </div>
      </div>
      ...
      <div className="p-4 border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between items-center transition-colors">
        <p className="text-xs text-gray-500 dark:text-gray-400">
            Showing <span className="font-bold text-gray-900 dark:text-white">{(currentPage - 1) * itemsPerPage + 1}</span> - <span className="font-bold text-gray-900 dark:text-white">{Math.min(currentPage * itemsPerPage, groupedData.length)}</span> of <span className="font-bold text-gray-900 dark:text-white">{groupedData.length}</span>
        </p>
