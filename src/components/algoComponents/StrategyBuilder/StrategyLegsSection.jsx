...
import React, { useEffect, useRef, useState } from 'react';
import { Plus, Trash2, Minus, Copy, AlertCircle, Edit, CandlestickChart } from 'lucide-react';
import ComingSoonOverlay from './ComingSoonOverlay';
import { EXPIRY_TYPES, STRIKE_CRITERIA, ATM_POINT_STEPS, ATM_PERCENT_STEPS } from '../../../data/instrumentData';
...
const StrategyLegsSection = ({ legs, addLeg, updateLeg, removeLeg, isComingSoon, strategyType, instruments, advanceSettings, entrySettings }) => {
  ...
  useEffect(() => {
    if (hasInstrument && legs.length > 0) {
      legs.forEach(leg => {
        if (leg.quantity !== baseLotSize) updateLeg(leg.id, 'quantity', baseLotSize);
      });
    }
  }, [baseLotSize]);
  ...
  const getLegSummary = (leg) => {
    ...
    return `${actionText}  ●  Qty ${leg.quantity}  ●  ${strikeText}  ●  TP ${leg.tpValue}${leg.tpType==='TP%'?'%':' Pt'}  ●  SL ${leg.slValue}${leg.slType==='SL%'?'%':' Pt'}`;
  };
  ...
  const renderStrikeTypeInput = (leg) => {
    const criteria = leg.strikeCriteria || "ATM pt";
    const inputClass = "w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors";
    
    if (criteria === "ATM pt" || criteria === "ATM %") { 
        const options = criteria === "ATM pt" ? ["ITM 100", "ATM", "OTM 100"] : ["ITM 5%", "ATM", "OTM 5%"]; // Simplified for example
    ...
    return (
        <div className="flex items-center bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded overflow-hidden">
            <button onClick={() => handleValueChange(leg.id, 'strikeType', leg.strikeType, -1, criteria)} className="px-2 py-1.5 text-gray-500 hover:bg-gray-100 border-r dark:border-slate-600"><Minus size={12}/></button>
            <input type="number" value={leg.strikeType} onChange={(e) => updateLeg(leg.id, 'strikeType', e.target.value)} className="w-full text-center bg-transparent text-xs outline-none" />
            <button onClick={() => handleValueChange(leg.id, 'strikeType', leg.strikeType, 1, criteria)} className="px-2 py-1.5 text-gray-500 hover:bg-gray-100 border-l dark:border-slate-600"><Plus size={12}/></button>
        </div>
    );
  };

  // ✅ Check if any Per-Leg Advance feature is active
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-200 dark:border-slate-700 relative h-full min-h-[600px] flex flex-col shadow-sm transition-colors duration-300">
      
      {isComingSoon && <ComingSoonOverlay />}
      {!isComingSoon && !hasInstrument && (
        <div className="absolute inset-0 z-40 bg-gray-50/80 dark:bg-slate-900/60 backdrop-blur-sm flex justify-center items-center rounded-xl border border-gray-200 dark:border-slate-700/50">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border shadow-xl flex flex-col items-center gap-3 text-center">
                <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center text-blue-600"><AlertCircle size={28} /></div>
                <div><h4 className="font-bold text-lg">Select Instrument</h4><p className="text-gray-500 text-xs mt-1">Please add an instrument to begin.</p></div>
            </div>
        </div>
      )}

      {/* 🟢 HEADER (Rule A Implemented: Hide Add Leg if moveSLToCost is ON) */}
      <div className="flex justify-between items-center mb-5">
        <h3 className="font-bold text-lg text-gray-800 dark:text-white">Strategy Legs</h3>
        {(!advanceSettings?.moveSLToCost) && (
            <button onClick={() => addLeg()} className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 shadow-md shadow-blue-500/20 active:scale-95 transition-all"><Plus size={14} /> Add Leg</button>
        )}
      </div>
      ...
      <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar pb-4">
        {legs.map((leg, index) => {
          ...
          return (
            <div key={leg.id} className={`rounded-lg relative group transition-all border shadow-sm ${isExpanded ? 'bg-gray-50 dark:bg-slate-900 border-gray-300 dark:border-slate-700' : 'bg-white dark:bg-slate-900 border-gray-200 hover:border-gray-300'}`}>
              
              {/* Leg Header */}
              <div className={`flex justify-between items-center p-3 ${isExpanded ? 'border-b border-gray-300 dark:border-slate-700' : ''}`}>
                 <div className="flex-1 cursor-pointer" onClick={() => setExpandedLegId(leg.id)}>
                    <h4 className="font-bold text-sm flex items-center gap-2 dark:text-white">Leg {index + 1} 
                      ...
                      {advanceSettings?.waitAndTrade && (
                          <div className="grid grid-cols-2 gap-4 mt-3 bg-amber-50/50 dark:bg-amber-900/10 p-3 rounded-lg border border-amber-100 dark:border-amber-800/30">
                              <div>
                                  ...
                                  <select value={leg.waitAndTradeType || advanceSettings.waitAndTradeConfig?.type || '%'} onChange={e => updateLeg(leg.id, 'waitAndTradeType', e.target.value)} className={inputClass}>
                                      <option value="%">% ↑</option>
                              ...
                              <div>
                                  ...
                                  <input type="number" value={leg.waitAndTradeMovement !== undefined ? leg.waitAndTradeMovement : (advanceSettings.waitAndTradeConfig?.movement || 0)} onChange={e => updateLeg(leg.id, 'waitAndTradeMovement', e.target.value)} className={inputClass} />
                              </div>
                          </div>
                      )}
                      ...
                      {advanceSettings?.premiumDifference && (
                          <div className="grid grid-cols-2 gap-4 mt-3 bg-teal-50/50 dark:bg-teal-900/10 p-3 rounded-lg border border-teal-100 dark:border-teal-800/30">
                              <div>
                                  ...
                                  <input type="number" value={leg.premiumDifference !== undefined ? leg.premiumDifference : (advanceSettings.premiumDifferenceConfig?.premium || 0)} onChange={e => updateLeg(leg.id, 'premiumDifference', e.target.value)} className={inputClass} />
                              </div>
                          </div>
                      )}
                      ...
                      {advanceSettings?.reEntryExecute && (
                          <div className="grid grid-cols-3 gap-4 mt-3 bg-blue-50/50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-100 dark:border-blue-800/30">
                              <div>
                                  ...
                                  <select value={leg.reEntryType || advanceSettings.reEntryExecuteConfig?.reEntryType || 'ReExecute'} onChange={e => updateLeg(leg.id, 'reEntryType', e.target.value)} className={inputClass}>
                              ...
                              <div>
                                  ...
                                  <select value={leg.actionType || advanceSettings.reEntryExecuteConfig?.actionType || 'On Close'} onChange={e => updateLeg(leg.id, 'actionType', e.target.value)} className={inputClass}>
                              ...
                              <div>
                                  ...
                                  <input type="number" value={leg.reEntryCycles !== undefined ? leg.reEntryCycles : (advanceSettings.reEntryExecuteConfig?.cycles || 0)} onChange={e => updateLeg(leg.id, 'reEntryCycles', e.target.value)} className={inputClass} />
                              </div>
                          </div>
                      )}
                      ...
                      {advanceSettings?.trailSL && (
                          <div className="grid grid-cols-3 gap-4 mt-3 bg-pink-50/50 dark:bg-pink-900/10 p-3 rounded-lg border border-pink-100 dark:border-pink-800/30">
                              <div>
                                  ...
                                  <select value={leg.trailSLType || advanceSettings.trailSLConfig?.trailType || 'Pt'} onChange={e => updateLeg(leg.id, 'trailSLType', e.target.value)} className={inputClass}>
                                      <option value="%">%</option>
                                      <option value="Pt">Pt</option>
                                  </select>
                              </div>
                              <div>
                                  ...
                                  <input type="number" value={leg.trailSLMovement !== undefined ? leg.trailSLMovement : (advanceSettings.trailSLConfig?.x || 0)} onChange={e => updateLeg(leg.id, 'trailSLMovement', e.target.value)} className={inputClass} />
                              </div>
                              <div>
                                  <label className="text-[11px] text-pink-700 dark:text-pink-500 font-bold block mb-1.5">Trailing Value</label>
                                  <input type="number" value={leg.trailSLValue !== undefined ? leg.trailSLValue : (advanceSettings.trailSLConfig?.y || 0)} onChange={e => updateLeg(leg.id, 'trailSLValue', e.target.value)} className={inputClass} />
                              </div>
                          </div>
                      )}
