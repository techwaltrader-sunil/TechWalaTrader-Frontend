...
import {
  getDefaultExpiry,
  getAllowedExpiries,
} from "../../../data/InstrumentsExpiryList";

const StrategyLegsSection = ({
  config,
  legs,
  addLeg,
  updateLeg,
  ...
  const hasInstrument = instruments && instruments.length > 0;
  const selectedInstrumentName = hasInstrument
    ? instruments[0].name
    : "NIFTY 50";
  const baseLotSize = hasInstrument ? instruments[0].lot || 1 : 1;
  ...
  const handleSignalChange = (field, value) => {
    if (setEntrySettings) {
      setEntrySettings((prev) => {
        const currentConfig = prev.signalCandle || {
          enabled: false,
          tradeOnTrigger: false,
          buyWhen: "High Break",
          shortWhen: "Low Break",
          continuousCandle: false,
        };
        const updates = { [field]: value };
  ...
  const getLegSummary = (leg) => {
    ...
    let strikeText = "";
    const sType =
      leg.strikeType !== undefined && leg.strikeType !== null
        ? leg.strikeType
        : "ATM";
    const sCriteria = leg.strikeCriteria || "ATM pt";
    if (sType === "ATM") {
      ...
      if (sType.includes("ITM"))
        strikeText = `Strike ATM -${sType.replace("ITM ", "")} pts`;
      else if (sType.includes("OTM"))
        strikeText = `Strike ATM +${sType.replace("OTM ", "")} pts`;
      else strikeText = `Strike ${sCriteria} ${sType}`;
    } else if (sCriteria === "ATM %" && typeof sType === "string") {
      const val = sType.replace("ITM ", "").replace("OTM ", "");
      if (sType.includes("ITM")) strikeText = `Strike ATM -${val}`;
      ...
      strikeText = `Strike ${sCriteria} ${sType === "" ? "?" : sType}`;
    }
    const currentSLType = leg.slType || "SL%";
    const currentTPType = leg.tpType || "TP%";
    const slLabel = currentSLType === "SL%" ? "%" : " Pts";
  ...
  const handleIncrement = (legId, currentQty) =>
    updateLeg(legId, "quantity", (currentQty || 0) + baseLotSize);
  const handleDecrement = (legId, currentQty) =>
    updateLeg(legId, "quantity", Math.max(0, (currentQty || 0) - baseLotSize));
  const handleValueChange = (legId, field, currentValue, change, criteria) => {
    let base = currentValue;
    if (base === "" || base === null || base === undefined) base = 0;
    const safeValue = parseFloat(base);
    const newValue = safeValue + change;
    if ((criteria === "CP >=" || criteria === "CP <=") && newValue < 0) {
      updateLeg(legId, field, 0);
      return;
    }
    updateLeg(legId, field, parseFloat(newValue.toFixed(2)));
  };
  ...
  const renderStrikeTypeInput = (leg) => {
    const criteria = leg.strikeCriteria || "ATM pt";
    const generateStrikePoints = () => {
      const step = ATM_POINT_STEPS[selectedInstrumentName] || 50;
      const maxRange = 2000;
      let options = [];
      for (let i = maxRange; i >= step; i -= step) options.push(`ITM ${i}`);
      options.push("ATM");
      for (let i = step; i <= maxRange; i += step) options.push(`OTM ${i}`);
      return options;
    };
    const generateStrikePercents = () => {
      const step = ATM_PERCENT_STEPS[selectedInstrumentName] || 1.0;
      const maxRange = 20.0;
      let options = [];
      for (let i = maxRange; i >= step; i -= step)
        options.push(`ITM ${i.toFixed(1)}%`);
      options.push("ATM");
      for (let i = step; i <= maxRange; i += step)
        options.push(`OTM ${i.toFixed(1)}%`);
      return options;
    };
    const inputClass =
      "w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none custom-scrollbar focus:border-blue-500 transition-colors";

    if (criteria === "ATM pt" || criteria === "ATM %") {
      const options =
        criteria === "ATM pt"
          ? generateStrikePoints()
          : generateStrikePercents();
      return (
        <select
    ...
    if (criteria === "Delta") {
      return (
        <input
          ...
          max="1"
          placeholder="0.5"
          value={typeof leg.strikeType === "number" ? leg.strikeType : ""}
          onChange={(e) => {
            ...
            updateLeg(leg.id, "strikeType", numVal);
          }}
          className={inputClass}
        />
      );
    ...
    if (criteria === "CP") {
      return (
        <div className="flex items-center bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded overflow-hidden transition-colors">
          <button
            onClick={() =>
              handleValueChange(
                leg.id,
            ...
            className="px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 dark:text-gray-400 border-r border-gray-300 dark:border-slate-600"
          >
            <Minus size={12} />
          </button>
          <input
            ...
            onChange={(e) => updateLeg(leg.id, "strikeType", e.target.value)}
            className="w-full text-center bg-transparent text-xs text-gray-900 dark:text-gray-300 outline-none appearance-none placeholder-gray-400"
          />
          <button
            onClick={() =>
              handleValueChange(
            ...
            className="px-2 py-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-500 dark:text-gray-400 border-l border-gray-300 dark:border-slate-600"
          >
            <Plus size={12} />
          </button>
        </div>
      ...
      return (
        <div className="flex items-center bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded overflow-hidden transition-colors">
          ...
          <input
            ...
            value={leg.strikeType}
            onChange={(e) => updateLeg(leg.id, "strikeType", e.target.value)}
            className="w-full text-center bg-transparent text-xs text-gray-900 dark:text-gray-300 outline-none appearance-none"
          />
          <button
            onClick={() =>
              handleValueChange(
  ...
  const hasInlineAdvanceFeatures =
    advanceSettings?.waitAndTrade ||
    advanceSettings?.premiumDifference ||
    advanceSettings?.reEntryExecute ||
    advanceSettings?.trailSL;
  ...
  const transactionType = config?.transactionType || "Both Side";
  const showLongCondition =
    transactionType === "Both Side" || transactionType === "Only Long";
  const canShowShortBasedOnChart =
    !entrySettings?.useCombinedChart && !entrySettings?.useOptionsChart;
  const showShortCondition =
    (transactionType === "Both Side" || transactionType === "Only Short") &&
    canShowShortBasedOnChart;

  const gridColsClass =
    showLongCondition && showShortCondition ? "grid-cols-2" : "grid-cols-1";
    ...
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-200 dark:border-slate-700 relative h-full min-h-[600px] flex flex-col shadow-sm dark:shadow-none transition-colors duration-300">
      ...
      <div className="flex justify-between items-center mb-5">
        ...
        {!advanceSettings?.moveSLToCost && (
          <button
            onClick={() => addLeg()}
            className="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded text-xs font-bold flex items-center gap-1 shadow-md shadow-blue-500/20 transition-all"
          >
            <Plus size={14} /> Add Leg
          </button>
        )}
      ...
      <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar pb-4">
        {legs.map((leg, index) => {
          ...
          return (
            <div
              key={leg.id}
              className={`rounded-lg relative transition-all border shadow-sm 
                ${
                  isExpanded
                    ? "bg-gray-50 dark:bg-slate-900 border-gray-300 dark:border-slate-700"
                    : "bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-800 hover:border-gray-300 dark:hover:border-slate-700"
            ...
            >
              <div
                className={`flex justify-between items-center p-3 ${isExpanded ? "border-b border-gray-300 dark:border-slate-700" : ""}`}
              >
                <div
                  className="flex-1 cursor-pointer"
                  onClick={() => setExpandedLegId(leg.id)}
                >
                  <h4 className="font-bold text-gray-800 dark:text-gray-200 text-sm flex items-center gap-2">
                    Leg {index + 1}
                    <span
                      className={`text-xs font-bold px-1.5 rounded ${leg.action === "BUY" ? "text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-500/20" : "text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-500/20"}`}
                    >
                      {leg.action} {displayTypeLabel}
                    </span>
                  </h4>
                ...
                <div className="flex items-center gap-2 ml-3">
                  {!isExpanded && (
                    <button
                      onClick={() => setExpandedLegId(leg.id)}
                      className="bg-white dark:bg-slate-800 hover:bg-blue-600 hover:text-white text-blue-600 dark:text-blue-400 px-3 py-1.5 rounded text-[10px] font-bold border border-gray-300 dark:border-slate-700 transition-colors flex items-center gap-1"
                    >
                      <Edit size={12} /> Edit
                    </button>
                  )}
                  {isExpanded && (
                    <span className="text-[10px] font-bold text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-500/10 px-2 py-0.5 rounded-full border border-green-200 dark:border-green-500/20 hidden sm:inline-block">
                      ACTIVE
                    </span>
                  )}
                  ...
                  <div className="p-4 space-y-4 animate-in slide-in-from-top-1 duration-200">
                    {strategyType === "Indicator Based" && (
                      <div className={`grid ${gridColsClass} gap-4`}>
                        ...
                        {showLongCondition && (
                          <div>
                            <label className="text-[11px] text-green-600 dark:text-green-500 font-bold block mb-1.5">
                              When Long Condition
                            </label>
                            <select
                              className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-green-500 transition-colors"
                        ...
                        {showShortCondition && (
                          <div>
                            <label className="text-[11px] text-red-600 dark:text-red-500 font-bold block mb-1.5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {/* Quantity */}
                      <div>
                        <label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">
                          Qty
                        </label>
                        <div className="flex items-center">
                          <button
                            onClick={() =>
                              handleDecrement(leg.id, leg.quantity)
                            }
                            className="bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 px-3 py-1.5 rounded-l border border-gray-300 dark:border-slate-600 font-bold transition-colors"
                          >
                            <Minus size={14} />
                          </button>
                          <input
                            ...
                            value={leg.quantity}
                            readOnly
                            className="w-full text-center bg-white dark:bg-slate-800 border-y border-gray-300 dark:border-slate-600 py-1.5 text-sm text-gray-900 dark:text-slate-100 font-bold outline-none transition-colors"
                          />
                          <button
                            onClick={() =>
                              handleIncrement(leg.id, leg.quantity)
                            }
                            className="bg-gray-100 hover:bg-gray-200 dark:bg-slate-800 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-400 px-3 py-1.5 rounded-r border border-gray-300 dark:border-slate-600 font-bold transition-colors"
                          >
                            <Plus size={14} />
                          </button>
                        </div>
                        <p className="text-[9px] text-gray-400 dark:text-gray-400 mt-1">
                          Multiples of lot ({baseLotSize})
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex rounded-md overflow-hidden border border-gray-300 dark:border-slate-600">
                          <button
                            ...
                            className={`flex-1 py-1.5 text-xs font-bold transition-colors ${leg.action === "BUY" ? "bg-green-100 text-green-700 dark:bg-green-600/20 dark:text-green-500" : "bg-gray-50 dark:bg-slate-800 text-gray-500"}`}
                          >
                            BUY
                          </button>
                          <button
                            onClick={() => updateLeg(leg.id, "action", "SELL")}
                            className={`flex-1 py-1.5 text-xs font-bold transition-colors ${leg.action === "SELL" ? "bg-red-100 text-red-700 dark:bg-red-600/20 dark:text-red-500" : "bg-gray-50 dark:bg-slate-800 text-gray-500"}`}
                          >
                            SELL
                          </button>
                        </div>
                      ...
                      {strategyType !== "Indicator Based" && (
                        <div>
                          ...
                          <div className="flex rounded-md overflow-hidden border border-gray-300 dark:border-slate-600">
                            <button
                              onClick={() =>
                                updateLeg(leg.id, "optionType", "Call")
                              }
                              className={`flex-1 py-1.5 text-xs font-bold transition-colors ${leg.optionType === "Call" ? "bg-blue-100 text-blue-700 dark:bg-blue-600/20 dark:text-blue-500" : "bg-gray-50 dark:bg-slate-800 text-gray-500"}`}
                            >
                              Call
                            </button>
                            <button
                              onClick={() =>
                                updateLeg(leg.id, "optionType", "Put")
                              }
                              className={`flex-1 py-1.5 text-xs font-bold transition-colors ${leg.optionType === "Put" ? "bg-blue-100 text-blue-700 dark:bg-blue-600/20 dark:text-blue-500" : "bg-gray-50 dark:bg-slate-800 text-gray-500"}`}
                            >
                              Put
                            </button>
                          </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        {/* Label with Strike Criteria Tooltip */}
                        <div className="flex items-center gap-1.5 mb-1.5 relative group w-max">
                          ...
                          {/* 🌟 Smart Tooltip Box (Original Centered Style) 🌟 */}
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex flex-col w-[300px] bg-gray-900 dark:bg-slate-800 border border-gray-700 shadow-xl rounded-lg p-3 z-50 text-[11px] text-gray-200 pointer-events-auto transition-all animate-in fade-in zoom-in-95 duration-200">
                            {/* Language Toggle Header */}
                            <div className="flex justify-between items-center mb-2 pb-2 border-b border-gray-700">
                              <span className="font-bold text-gray-400">
                                Strike Info / स्ट्राइक जानकारी
                              </span>
                              ...
                              <div className="flex bg-gray-800 rounded p-0.5 border border-gray-600">
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded text-[10px] font-bold transition-colors ${criteriaLang === "en" ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white"}`}
                                >
                                  EN
                                </button>
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded text-[10px] font-bold transition-colors ${criteriaLang === "hi" ? "bg-blue-600 text-white" : "text-gray-400 hover:text-white"}`}
                                >
                                  HI
                                </button>
                              </div>
                            ...
                            {criteriaLang === "hi" ? (
                              <div className="space-y-2">
                                <div>
                                  ...
                                  <span className="text-gray-300 block mt-0.5 leading-relaxed">
                                    लाइव मार्केट भाव (Spot Price) से दूरी के
                                    हिसाब से स्ट्राइक चुनें (जैसे ITM 100, OTM
                                    200)।
                                  </span>
                                </div>
                                <div className="border-t border-gray-700/50 pt-2">
                                  ...
                                  <span className="text-gray-300 block mt-0.5 leading-relaxed">
                                    ऑप्शन की स्पीड। <b>0.4 Delta</b> का मतलब है
                                    कि अगर BankNifty 1 पॉइंट बढ़ेगा, तो आपका
                                    प्रीमियम 0.40 पैसे बढ़ेगा। (SMC ट्रेडर्स की
                                    पसंद)
                                  </span>
                                </div>
                                <div className="border-t border-gray-700/50 pt-2">
                                  ...
                                  <span className="text-gray-300 block mt-0.5 leading-relaxed">
                                    Closest Premium. वो स्ट्राइक चुनें जिसका
                                    प्रीमियम आपके डाले गए भाव (जैसे ₹150) के
                                    सबसे करीब हो।
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="space-y-2">
                                <div>
                                  ...
                                  <span className="text-gray-300 block mt-0.5 leading-relaxed">
                                    Select strike based on distance from the
                                    current Spot Price (e.g., ITM 100, OTM 200).
                                  </span>
                                </div>
                                <div className="border-t border-gray-700/50 pt-2">
                                  <strong className="text-purple-400 flex items-center gap-1">
                                  ...
                                  <span className="text-gray-300 block mt-0.5 leading-relaxed">
                                    Option speed. <b>0.4 Delta</b> means if
                                    BankNifty moves 1 pt, your premium moves
                                    0.40 pts. (Smart Money choice)
                                  </span>
                                </div>
                                <div className="border-t border-gray-700/50 pt-2">
                                  <strong className="text-green-400 flex items-center gap-1">
                                  ...
                                  <span className="text-gray-300 block mt-0.5 leading-relaxed">
                                    Closest Premium. Selects the strike whose
                                    premium is closest to your target value
                                    (e.g., ₹150).
                                  </span>
                                </div>
                              </div>
                            ...
                            {/* Tooltip Arrow */}
                            <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-gray-900 dark:bg-slate-800 border-b border-r border-gray-700 rotate-45"></div>
                          </div>
                        </div>
                        
                        {/* 🔥 YAHAN SE DUSRA LABEL HATA DIYA GAYA HAI 🔥 */}
                        ...
                        >
                          {STRIKE_CRITERIA.map((cri) => (
                            <option key={cri}>{cri}</option>
                          ))}
                        </select>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <select
                          ...
                          value={leg.slType || "SL%"}
                          onChange={(e) =>
                            updateLeg(leg.id, "slType", e.target.value)
                          }
                        >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={leg.slValue}
                          onChange={(e) =>
                            updateLeg(leg.id, "slValue", e.target.value)
                          }
                          className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none transition-colors"
                        />
                      </div>
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold">
                            Execution
                          </label>
                              ...
                              <div className="flex justify-between items-center mb-2 pb-1.5 border-b border-gray-100 dark:border-slate-700">
                                ...
                                <div className="flex bg-gray-100 dark:bg-slate-900 rounded p-0.5 border border-gray-200 dark:border-slate-700">
                                  <button
                                    ...
                                    className={`px-1.5 py-0.5 rounded text-[9px] font-bold transition-colors ${tooltipLang === "en" ? "bg-blue-600 text-white" : "text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white"}`}
                                  >
                                    EN
                                  </button>
                                  <button
                                    ...
                                    className={`px-1.5 py-0.5 rounded text-[9px] font-bold transition-colors ${tooltipLang === "hi" ? "bg-blue-600 text-white" : "text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white"}`}
                                  >
                                    HI
                                  </button>
                                </div>
                              ...
                              {tooltipLang === "hi" ? (
                                <>
                                  <div className="mb-1.5 pb-1.5 border-b border-gray-100 dark:border-slate-700/50">
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      स्कैल्पर्स (Scalpers) के लिए। भाव (LTP) टच
                                      होते ही तुरंत SL कट जाएगा।
                                    </span>
                                  </div>
                                  <div>
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      ट्रेंड फॉलोअर्स के लिए। कैंडल क्लोज होने
                                      का इंतज़ार करेगा ताकि फेक शैडो (Wicks) से
                                      SL हिट ना हो।
                                    </span>
                                  </div>
                                </>
                                ...
                                <>
                                  <div className="mb-1.5 pb-1.5 border-b border-gray-100 dark:border-slate-700/50">
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      For Scalpers. Exits instantly when LTP
                                      touches your SL.
                                    </span>
                                  </div>
                                  <div>
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      For Trend followers. Waits for candle
                                      close to prevent SL hunting by wicks.
                                    </span>
                                  </div>
                                </>
                              )}
                        ...
                        <select
                          ...
                          value={leg.slExecuteType || "On Price"}
                          onChange={(e) =>
                            updateLeg(leg.id, "slExecuteType", e.target.value)
                          }
                        >
                          <option value="On Price">On Price</option>
                          <option value="On Close">On Close</option>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <select
                          ...
                          value={leg.tpType || "TP%"}
                          onChange={(e) =>
                            updateLeg(leg.id, "tpType", e.target.value)
                          }
                        >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={leg.tpValue}
                          onChange={(e) =>
                            updateLeg(leg.id, "tpValue", e.target.value)
                          }
                          className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none transition-colors"
                        />
                      </div>
                      ...
                      <div>
                              ...
                              {tooltipLang === "hi" ? (
                                <>
                                  <div className="mb-1.5 pb-1.5 border-b border-gray-100 dark:border-slate-700/50">
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      भाव (LTP) टारगेट टच करते ही तुरंत प्रॉफिट
                                      बुक हो जाएगा।
                                    </span>
                                  </div>
                                  <div>
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      कैंडल के टारगेट के ऊपर क्लोज होने का
                                      इंतज़ार करेगा। (बड़े ट्रेंड्स कैप्चर करने के
                                      लिए)
                                    </span>
                                  </div>
                                </>
                                ...
                                <>
                                  <div className="mb-1.5 pb-1.5 border-b border-gray-100 dark:border-slate-700/50">
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      Books profit instantly when LTP touches
                                      your target.
                                    </span>
                                  </div>
                                  <div>
                                    ...
                                    <span className="text-gray-600 dark:text-gray-300 block mt-0.5 leading-relaxed">
                                      Waits for candle to close above target.
                                      (Good for capturing bigger trends)
                                    </span>
                                  </div>
                                </>
                              )}
                        ...
                        <select
                          ...
                          value={leg.tpExecuteType || "On Price"}
                          onChange={(e) =>
                            updateLeg(leg.id, "tpExecuteType", e.target.value)
                          }
                        >
                          <option value="On Price">On Price</option>
                          <option value="On Close">On Close</option>
                    ...
                    {hasInlineAdvanceFeatures && (
                      <div className="mt-6 mb-2">
                        <div className="flex items-center justify-center opacity-60">
                          <div className="h-px bg-gray-300 dark:bg-gray-600 w-full"></div>
                          <span className="text-[10px] text-gray-500 dark:text-gray-400 px-3 uppercase font-bold whitespace-nowrap">
                            --- Advance Features ---
                          </span>
                          <div className="h-px bg-gray-300 dark:bg-gray-600 w-full"></div>
                        </div>
                    ...
                    {advanceSettings?.waitAndTrade && (
                      <div className="grid grid-cols-2 gap-4 mt-3 bg-amber-50/50 dark:bg-amber-900/10 p-3 rounded-lg border border-amber-100 dark:border-amber-800/30">
                        <div>
                          <label className="text-[11px] text-amber-700 dark:text-amber-500 font-bold block mb-1.5">
                            Wait & Trade Type
                          </label>
                          <select
                            value={
                              leg.waitAndTradeType ||
                              advanceSettings.waitAndTradeConfig?.type ||
                              "%"
                            }
                            onChange={(e) =>
                              updateLeg(
                        ...
                        <div>
                          ...
                          <input
                            type="number"
                            value={
                              leg.waitAndTradeMovement !== undefined
                                ? leg.waitAndTradeMovement
                                : advanceSettings.waitAndTradeConfig
                                    ?.movement || 0
                            }
                            onChange={(e) =>
                              updateLeg(
                                leg.id,
                                "waitAndTradeMovement",
                                e.target.value,
                              )
                            }
                            className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors"
                    ...
                    {advanceSettings?.premiumDifference && (
                      <div className="grid grid-cols-2 gap-4 mt-3 bg-teal-50/50 dark:bg-teal-900/10 p-3 rounded-lg border border-teal-100 dark:border-teal-800/30">
                        <div>
                          <label className="text-[11px] text-teal-700 dark:text-teal-500 font-bold block mb-1.5">
                            Premium Difference
                          </label>
                          <input
                            type="number"
                            value={
                              leg.premiumDifference !== undefined
                                ? leg.premiumDifference
                                : advanceSettings.premiumDifferenceConfig
                                    ?.premium || 0
                            }
                            onChange={(e) =>
                              updateLeg(
                    ...
                    {advanceSettings?.reEntryExecute && (
                      <div className="grid grid-cols-3 gap-4 mt-3 bg-blue-50/50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-100 dark:border-blue-800/30">
                        <div>
                          <label className="text-[11px] text-blue-700 dark:text-blue-500 font-bold block mb-1.5">
                            Re-Entry Type
                          </label>
                          <select
                            value={
                              leg.reEntryType ||
                              advanceSettings.reEntryExecuteConfig
                                ?.reEntryType ||
                              "ReExecute"
                            }
                            onChange={(e) =>
                              updateLeg(leg.id, "reEntryType", e.target.value)
                            }
                            className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors"
                          >
                            <option>ReExecute</option>
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateLeg(leg.id, "actionType", e.target.value)
                            }
                            className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors"
                          >
                            <option>On Close</option>
                        ...
                        <div>
                          ...
                          <input
                            type="number"
                            value={
                              leg.reEntryCycles !== undefined
                                ? leg.reEntryCycles
                                : advanceSettings.reEntryExecuteConfig
                                    ?.cycles || 0
                            }
                            onChange={(e) =>
                              updateLeg(leg.id, "reEntryCycles", e.target.value)
                            }
                            className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors"
                          />
                        </div>
                    ...
                    {advanceSettings?.trailSL && (
                      <div className="grid grid-cols-3 gap-4 mt-3 bg-pink-50/50 dark:bg-pink-900/10 p-3 rounded-lg border border-pink-100 dark:border-pink-800/30">
                        <div>
                          <label className="text-[11px] text-pink-700 dark:text-pink-500 font-bold block mb-1.5">
                            Trail SL Type
                          </label>
                          <select
                            value={
                              leg.trailSLType ||
                              advanceSettings.trailSLConfig?.trailType ||
                              "Pt"
                            }
                            onChange={(e) =>
                              updateLeg(leg.id, "trailSLType", e.target.value)
                            }
                            className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors"
                          >
                            <option value="%">%</option>
                        ...
                        <div>
                          ...
                          <input
                            type="number"
                            value={
                              leg.trailSLMovement !== undefined
                                ? leg.trailSLMovement
                                : advanceSettings.trailSLConfig?.x || 0
                            }
                            onChange={(e) =>
                              updateLeg(
                                leg.id,
                        ...
                        <div>
                          ...
                          <input
                            type="number"
                            value={
                              leg.trailSLValue !== undefined
                                ? leg.trailSLValue
                                : advanceSettings.trailSLConfig?.y || 0
                            }
                            onChange={(e) =>
                              updateLeg(leg.id, "trailSLValue", e.target.value)
                            }
                            className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none focus:border-blue-500 transition-colors"
                          />
                        </div>
                  ...
                  <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-800 rounded-b-lg border-t border-gray-200 dark:border-gray-500 transition-colors">
                    {strategyType !== "Time Based" && (
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span className="text-[10px] text-gray-500 dark:text-gray-500 font-bold">
                          Pre Punch SL
                        </span>
                      </label>
                    )}
                      ...
                      <button
                        ...
                        onClick={() => {
                          addLeg({
                            ...leg,
                            optionType:
                              leg.optionType === "Call" ? "Put" : "Call",
                            longCondition:
                              leg.longCondition === "CE" ? "PE" : "CE",
                            shortCondition:
                              leg.shortCondition === "PE" ? "CE" : "PE",
                          });
                        }}
                      >
                        <Copy
        ...
        <div className="mt-4 p-4 border border-blue-200 dark:border-blue-500/30 bg-blue-50/50 dark:bg-slate-900/50 rounded-lg animate-in fade-in transition-colors">
          <div className="flex items-center justify-between mb-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                checked={isScEnabled}
                onChange={(e) =>
                  handleSignalChange("enabled", e.target.checked)
                }
              />
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-gray-800 dark:text-gray-200">
                  Add Signal Candle Condition (Optional)
                </span>
              </div>
            </label>
          </div>
          ...
          {isScEnabled && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 pl-6 border-l-2 border-blue-500/20">
              <div className="col-span-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    checked={scConfig.tradeOnTrigger || false}
                    onChange={(e) =>
                      handleSignalChange("tradeOnTrigger", e.target.checked)
                    }
                  />
              ...
              <div>
                <label className="text-[11px] text-gray-500 dark:text-gray-400 font-bold block mb-1.5">
                  Buy When
                </label>
                <select
                  className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                  value={scConfig.buyWhen || "High Break"}
                  onChange={(e) =>
                    handleSignalChange("buyWhen", e.target.value)
                  }
                >
                  <option value="High Break">High Break</option>
                  <option value="Low Break">Low Break</option>
              ...
              <div>
                ...
                <select
                  ...
                  value={scConfig.shortWhen || "Low Break"}
                  onChange={(e) =>
                    handleSignalChange("shortWhen", e.target.value)
                  }
                >
                  <option value="Low Break">Low Break</option>
                  <option value="High Break">High Break</option>
              ...
              <div className="col-span-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    checked={scConfig.continuousCandle || false}
                    onChange={(e) =>
                      handleSignalChange("continuousCandle", e.target.checked)
                    }
                  />
                  <span className="text-[11px] text-gray-500 dark:text-gray-400">
