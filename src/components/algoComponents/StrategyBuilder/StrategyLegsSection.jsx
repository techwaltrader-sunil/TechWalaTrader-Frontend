...
const StrategyLegsSection = ({ legs, addLeg, updateLeg, removeLeg, isComingSoon, strategyType, instruments, advanceSettings, entrySettings }) => {
  ...
  // ✅ Common Input Class used across the component
  const inputClass = "w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-gray-300 outline-none custom-scrollbar focus:border-blue-500 transition-colors";
              ...
              {isExpanded && (
                        <>
                            <div className="p-4 space-y-4 animate-in slide-in-from-top-1 duration-200">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">Expiry</label><select className={inputClass} value={leg.expiry || 'WEEKLY'} onChange={(e) => updateLeg(leg.id, 'expiry', e.target.value)}>{EXPIRY_TYPES.map(exp => <option key={exp} value={exp}>{exp}</option>)}</select></div>
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">Strike Criteria</label><select className={inputClass} value={leg.strikeCriteria || "ATM pt"} onChange={(e) => { const newCriteria = e.target.value; let newInitialValue = "ATM"; if (newCriteria === 'Delta') newInitialValue = 0.5; if (newCriteria === 'CP') newInitialValue = ""; if (newCriteria.includes('CP')) newInitialValue = 0; updateLeg(leg.id, 'strikeCriteria', newCriteria); updateLeg(leg.id, 'strikeType', newInitialValue); }}>{STRIKE_CRITERIA.map(cri => <option key={cri}>{cri}</option>)}</select></div>
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">Strike Type</label>{renderStrikeTypeInput(leg)}</div>
                                </div>

                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">SL Type</label><select className={inputClass} value={leg.slType || 'SL%'} onChange={(e) => updateLeg(leg.id, 'slType', e.target.value)}><option value="SL%">SL%</option><option value="Points">Points</option></select></div>
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">SL</label><input type="number" value={leg.slValue} onChange={(e) => updateLeg(leg.id, 'slValue', e.target.value)} className={inputClass} /></div>
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">On Price</label><select className={inputClass}><option>On Price</option><option>On Trigger</option></select></div>
                                </div>

                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">TP Type</label><select className={inputClass} value={leg.tpType || 'TP%'} onChange={(e) => updateLeg(leg.id, 'tpType', e.target.value)}><option value="TP%">TP%</option><option value="Points">Points</option></select></div>
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">TP</label><input type="number" value={leg.tpValue} onChange={(e) => updateLeg(leg.id, 'tpValue', e.target.value)} className={inputClass} /></div>
                                    <div><label className="text-[11px] text-gray-500 dark:text-gray-500 font-bold block mb-1.5">On Price</label><select className={inputClass}><option>On Price</option><option>On Trigger</option></select></div>
                                </div>
