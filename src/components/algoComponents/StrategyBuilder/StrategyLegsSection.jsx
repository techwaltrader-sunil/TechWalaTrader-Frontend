...
const StrategyLegsSection = ({ legs, addLeg, updateLeg, removeLeg, isComingSoon, strategyType, instruments, advanceSettings, entrySettings }) => {
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-gray-200 relative h-full min-h-[600px] flex flex-col shadow-sm">
        ...
        {legs.map((leg, index) => {
          ...
          return (
            <div key={leg.id} className={`rounded-lg relative group transition-all border shadow-sm ${isExpanded ? 'bg-gray-50 border-gray-300' : 'bg-white border-gray-200'}`}>
              ...
              {/* Footer: Pre Punch & Clone */}
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded-b-lg border-t border-gray-200 transition-colors">
                  {strategyType !== 'Time Based' && (
                      <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" className="accent-blue-600 w-3.5 h-3.5"/>
                          <span className="text-[10px] text-gray-500 font-bold">Pre Punch SL</span>
                      </label>
                  )}
                  <div className="flex items-center gap-3 text-gray-400 ml-auto">
                      {/* 🔥 THE MAGIC FIX: Duplicate button now flips CE/PE automatically 🔥 */}
                      <button 
      ...
      {strategyType === 'Indicator Based' && (
        <div className="mt-4 p-4 border border-blue-200 bg-blue-50/50 rounded-lg animate-in fade-in transition-colors">
            <div className="flex items-center justify-between mb-3">
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" className="w-3.5 h-3.5 accent-blue-500 rounded" checked={showSignalCandle} onChange={(e) => setShowSignalCandle(e.target.checked)}/>
                    <div className="flex items-center gap-2"><CandlestickChart size={16} className="text-blue-500" /><span className="text-sm font-bold text-gray-800">Add Signal Candle Condition (Optional)</span></div>
                </label>
            </div>
            {showSignalCandle && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 pl-6 border-l-2 border-blue-500/20">
                    <div className="col-span-2"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" className="w-3 h-3 accent-blue-500" /><span className="text-[11px] text-gray-500">Trade on Trigger Candle</span></label></div>
                    <div><label className="text-[11px] text-gray-500 font-bold block mb-1.5">Buy When</label><select className="w-full bg-white border border-gray-300 rounded px-2 py-1.5 text-xs outline-none"><option>High Break</option><option>Close &gt; High</option></select></div>
                    <div><label className="text-[11px] text-gray-500 font-bold block mb-1.5">Short When</label><select className="w-full bg-white border border-gray-300 rounded px-2 py-1.5 text-xs outline-none"><option>Low Break</option><option>Close &lt; Low</option></select></div>
                    <div className="col-span-2"><label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" className="w-3 h-3 accent-blue-500" /><span className="text-[11px] text-gray-500">Of Continuous Candle</span></label></div>
                </div>
            )}
        </div>
      )}
