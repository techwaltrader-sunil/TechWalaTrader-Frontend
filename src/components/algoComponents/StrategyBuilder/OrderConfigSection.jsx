...
//          {/* 🔥 4. NEW: ONLY EXPIRY DAY TOGGLE (0-DTE) 🔥 */}
//          <div className="pt-3 border-t border-gray-100 dark:border-slate-700/60">
//             <label className="flex items-center justify-between cursor-pointer group">
//                <div className="flex flex-col">
//                   <span className="text-xs font-bold text-gray-800 dark:text-gray-200 flex items-center gap-1.5 transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400">
//                      🎯 Trade Only on Expiry Days (0-DTE)
//                   </span>
//                   <span className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 leading-tight">
//                      Engine will automatically skip non-expiry trading days.
//                   </span>
//                </div>
...
//                      checked={config.onlyExpiryDay || false}
//                      onChange={(e) => updateConfig('onlyExpiryDay', e.target.checked)}
//                   />
...
//             <div className="space-y-5 animate-in fade-in slide-in-from-top-2 pt-4 border-t border-gray-200 dark:border-slate-700/50">
//                 <div className="grid grid-cols-2 gap-4">
...
const OrderConfigSection = ({ config, setConfig, isComingSoon, strategyType, entrySettings }) => {
  // 🔥 NAYA STATE: Date input ko temporary hold karne ke liye
  const [tempDate, setTempDate] = useState('');
  const skippedDates = config.skippedDates || [];
  ...
  // 🔥 NAYE FUNCTIONS: Date ko Add aur Remove karne ke liye
  const handleAddDate = () => {
    if (!tempDate) return;
    if (!skippedDates.includes(tempDate)) {
      updateConfig('skippedDates', [...skippedDates, tempDate]);
    }
    setTempDate(''); // Input reset kar do
  };

  const handleRemoveDate = (dateToRemove) => {
    updateConfig('skippedDates', skippedDates.filter(d => d !== dateToRemove));
  };
  ...
  return (
    ...
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-200 dark:border-slate-700 relative h-full shadow-sm dark:shadow-none transition-colors duration-300">
      ...
      <div className="space-y-5">
         ...
         {/* 🔥 4. ONLY EXPIRY DAY TOGGLE (0-DTE) 🔥 */}
         <div className="pt-3 border-t border-gray-100 dark:border-slate-700/60">
            <label className="flex items-center justify-between cursor-pointer group">
               <div className="flex flex-col">
                  <span className="text-xs font-bold text-gray-800 dark:text-gray-200 flex items-center gap-1.5 transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400">
         ...
         {/* 🔥 5. NEW: EVENT / DATE SKIPPER 🔥 */}
         <div className="pt-2">
             
             {/* ✨ Premium Horizontal Fade Divider ✨ */}
             <div className="h-[1px] w-full bg-gradient-to-r from-transparent via-gray-300 dark:via-slate-600 to-transparent mb-4"></div>
             
             <label className="block mb-2">
                 <span className="text-xs font-bold text-gray-800 dark:text-gray-200 flex items-center gap-1.5">
                     📅 Skip Specific Dates
                 </span>
                 <span className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 block leading-tight">
                     Exclude high-volatility events (e.g., Budget, RBI Policy) from backtest.
                 </span>
             </label>
             ...
             <div className="flex gap-2 mb-2">
                 <input 
                     ...
                     className="flex-1 bg-gray-50 dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white focus:border-blue-500 focus:outline-none transition-colors"
                 />
                 <button 
                     type="button"
                     onClick={handleAddDate}
                     className="bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold px-4 py-1.5 rounded transition-colors"
                 >
                     Add
                 </button>
             </div>
             ...
             {skippedDates.length > 0 && (
                 <div className="flex flex-wrap gap-1.5 mt-2 p-2 bg-gray-50 dark:bg-slate-900/50 border border-gray-100 dark:border-slate-700/50 rounded-lg">
                     {skippedDates.map(date => (
                         <div key={date} className="flex items-center gap-1 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-400 text-[10px] font-medium px-2 py-1 rounded">
                             {date}
                             <button 
                                 onClick={() => handleRemoveDate(date)}
                                 className="hover:text-red-800 dark:hover:text-red-300 ml-1 font-bold text-xs leading-none"
                             >
                                 ×
                             </button>
                         </div>
