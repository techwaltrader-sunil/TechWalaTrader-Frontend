...
// // ✅ NAYA: removeLeg prop ko yahan receive kiya hai
// const AdvanceFeaturesSection = ({ advanceSettings, setAdvanceSettings, legs, addLeg, removeLeg }) => {
...
//       // 🔥 THE NEW FIX: Agar Move SL to Cost UNCHECK kiya aur 2 legs hain, to dusra leg delete kar do!
//       if (key === 'moveSLToCost' && legs?.length === 2 && removeLeg) {
//           removeLeg(legs[1].id); // dusre leg (index 1) ko hatao
//       }

//     } else {
//       // ✅ TURN ON LOGIC
        
//       // AUTO-DUPLICATE LOGIC: Move SL to Cost pe click karte hi 1st leg copy ho jayega (Opposite type ke sath)
//       if (key === 'moveSLToCost' && legs?.length === 1 && addLeg) {
//           const baseLeg = legs[0];
//           addLeg({
//               ...baseLeg,
...
//       if (['waitAndTrade', 'premiumDifference', 'reEntryExecute', 'trailSL'].includes(key)) {
//           setPopupFeature(key);
//           // 🔥 FIX: Default value set to '% ↑' to match the new 4 options
//           if(key === 'waitAndTrade') setTempConfig({ type: '% ↑', movement: 0 });
//           if(key === 'premiumDifference') setTempConfig({ premium: 0 });
//           if(key === 'reEntryExecute') setTempConfig({ reEntryType: 'ReExecute', actionType: 'On Close', cycles: 0 });
//           if(key === 'trailSL') setTempConfig({ trailType: 'Pt', x: 0, y: 0 });
...
//                               {/* 🔥 FIX: Added all 4 options for Algorooms exact match */}
//                               <select value={tempConfig.type} onChange={e => setTempConfig({...tempConfig, type: e.target.value})} className="w-full border rounded p-2 text-sm outline-none">
//                                   <option value="% ↑"> % ↑ </option>
//                                   <option value="% ↓"> % ↓ </option>
...
//                               <input type="number" value={tempConfig.movement} onChange={e => setTempConfig({...tempConfig, movement: e.target.value})} className="w-full border rounded p-2 text-sm outline-none" />
//                           </div>
//                       </div>
//                   )}
...
const AdvanceFeaturesSection = ({ advanceSettings, setAdvanceSettings, legs, addLeg, removeLeg }) => {
  ...
  const handleToggle = (key) => {
    if (advanceSettings && advanceSettings[key]) {
      ...
      setAdvanceSettings(prev => {
          ...
          // Agar Move SL to Cost band hua, to Independent Trailing bhi automatic band karo
          if (key === 'moveSLToCost') {
              newSettings.independentTrailing = false;
          }
          return newSettings;
      });
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-200 dark:border-slate-700 animate-in fade-in slide-in-from-right-4 shadow-sm transition-colors duration-300">
       ...
       {/* 🔥 NEW FEATURE: Independent Trailing Legs (V-Shape Recovery) */}
       {advanceSettings?.moveSLToCost && (
           <label className="flex items-start space-x-3 cursor-pointer mt-4 p-3 bg-blue-50 dark:bg-slate-800/50 rounded-lg border border-blue-100 dark:border-slate-700 transition-all group hover:bg-blue-100/50">
               <input 
                   type="checkbox" 
                   checked={!!advanceSettings?.independentTrailing}
                   onChange={(e) => setAdvanceSettings(prev => ({ ...prev, independentTrailing: e.target.checked }))}
                   className="form-checkbox h-4 w-4 text-blue-600 rounded mt-0.5 cursor-pointer"
               />
               <div className="flex flex-col">
                   <span className="text-sm font-bold text-blue-900 dark:text-blue-100">
                       Independent Trailing Legs (V-Shape Recovery)
                   </span>
                   <span className="text-xs text-blue-700/80 dark:text-blue-300/80 leading-snug mt-1 font-medium">
                       If ON, moving one leg's SL to profit (Trailing) will NOT move the other leg's SL to cost. Allows unhedged legs to float and capture market reversals.
                   </span>
               </div>
           </label>
       )}
